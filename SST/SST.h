...
Design Choices:
	...
	Will copy key for allocation reasons.
...
T SST<T>::add_to_hash_and_return_object(T add_object, char *key) {
...
bool SST<T>::add_to_hash(T add_object, char *key) {
...
T SST<T>::get_object(char *key) {
	...
	// returning null from internally crashes
	if(holder == NULL) { return NULL; } else { return holder; }
}

bool SST<T>::do_exist(char *key) {
...
int SST<T>::delete_object(char *key) {
...
int SST<T>::linear_search(linear_bookmark &bookmark, T &ret_object) {
...
return_values SST<T>::clean_init() {
...
return_values SST<T>::check_duplication(unsigned char index, char *key) {
...
T SST<T>::get_token_object(unsigned char index, char *key) {
...
return_values SST<T>::get_key(unsigned char &keyOut, char *key) {
...
return_values SST<T>::add_to_hash_line(unsigned char index, char *key_in, T object_to_add) {
	...
	if((entries+1)>max_entries) { cout << "SST: Max entries reached." << endl; return CONTERROR; }
...
return_values SST<T>::delete_hash_line(unsigned char index) {
...
return_values SST<T>::delete_whole_tables() {
